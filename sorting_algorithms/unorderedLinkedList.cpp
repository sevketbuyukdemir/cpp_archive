...
template <class generic_type>
generic_type unorderedLinkedList<generic_type>::get_first_node_information() {
    assert(first_pointer != NULL);
    return first_pointer -> information;
}

template <class generic_type>
generic_type unorderedLinkedList<generic_type>::get_last_node_information() {
...
template <class generic_type>
void unorderedLinkedList<generic_type>::insertion_sort() {
    /**
     * Time complexity :
     *      Worst case: O(n^2)
     *      Best case: O(n)
     ...
     */
    // sort pointers
    node<generic_type> *last_in_sort_pointer;
    node<generic_type> *first_out_of_sort_pointer;
    node<generic_type> *current_node;
    node<generic_type> *current_trail_node;

    last_in_sort_pointer = first_pointer;
    if (first_pointer == NULL) {
        cout << "Linked list is empty!" << endl;
    } else if (first_pointer -> pointer == NULL) {
        cout << "Linked list is ordered!" << endl;
    } else {
        while (last_in_sort_pointer -> pointer != NULL) {
            first_out_of_sort_pointer = last_in_sort_pointer ->pointer;
            if (first_out_of_sort_pointer -> information < first_pointer -> pointer) {
                last_in_sort_pointer -> pointer = first_out_of_sort_pointer -> pointer;
                first_out_of_sort_pointer -> pointer = first_pointer;
                first_pointer = first_out_of_sort_pointer;
            } else {
                current_trail_node = first_pointer;
                current_node = first_pointer -> pointer;
                while (current_node -> information < first_out_of_sort_pointer -> information) {
                    current_trail_node = current_node;
                    current_node = current_node -> pointer;
                }
                if (current_node != first_out_of_sort_pointer) {
                    last_in_sort_pointer -> pointer = first_out_of_sort_pointer -> pointer;
                    first_out_of_sort_pointer -> current;
                    current_trail_node -> pointer = first_out_of_sort_pointer;
                } else {
                    last_in_sort_pointer = last_in_sort_pointer -> pointer;
                }
            }
        }
    }
